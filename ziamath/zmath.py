...
class Math:
    ...
    def fromlatextext(
        ...
        texts = parts[::2]
        maths = [
            tex2mml(p.replace("$", ""), inline=not p.startswith("$$"))
            for p in parts[1::2]
        ]
        mathels = [
            ET.fromstring(m) for m in maths
        ]  # Convert to xml, but drop opening <math>
    ...
    def svgxml(self) -> ET.Element:
        ...
        if config.svg_style:
            ...
            style.text = config.svg_style
        if config.svg_defs:
            defs = ET.SubElement(svg, "defs")
            defs.append(ET.fromstring(config.svg_defs))
        ...
        if self.eqnumber is not None:
            ...
            x = 1
            viewbox = (
                f"{fmt(bbox.xmin-1)} {fmt(-bbox.ymax-1)} {fmt(width)} {fmt(height)}"
            )
    ...
    def getsize(self) -> tuple[float, float]:
        ...
        return (
            self.node.bbox.xmax - self.node.bbox.xmin,
        ...
        if valign == "bottom":
            ...
            ystart = (
                ...
                + (
                    lineheights[0]
                    + lineofsts[0]
                    - (len(lines) - 1) * self.size * self.linespacing
                    + lineofsts[-1]
                )
                / 2
            )
        ...
        if self.rotation:
            ...
            if self.rotation_mode == "default":
                dx = {
                    "left": x - bbox[0],
                    "right": x - bbox[1],
                    "center": x - (bbox[1] + bbox[0]) / 2,
                }.get(halign, 0)
                dy = {
                    "top": y - bbox[2],
                    "bottom": y - bbox[3],
                    "base": -sinth * dx,
                    "center": y - (bbox[3] + bbox[2]) / 2,
                }.get(valign, 0)
